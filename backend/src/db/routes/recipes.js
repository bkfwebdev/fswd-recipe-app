...
export function recipesRoutes(app) {
  app.get('/api/v1/recipes', async (req, res) => {
    const { sortBy, sortOrder, author, category } = req.query
    const options = { sortBy, sortOrder }
    try {
      if (author && category) {
        return res
          .status(400)
          .json({ error: 'query by either author or category, not both' })
      } else if (author) {
        return res.json(await listRecipesByAuthor(author, options))
      } else if (category) {
        return res.json(await listRecipesByCategory(category, options))
      } else {
